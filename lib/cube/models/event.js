...
Event.setProperties({
  ...
  to_wire: { value: function(){ var event = { _id: new ObjectID(this.time/1000), d: this.data, b: this.bins() }; if (this.id) event.d.id = this.id; return event; }},
...
function find(db, measurement, callback){
  var expression = measurement.expression,
      ...
      start  = ObjectID.createFromTime(measurement.start/1000),
      stop   = ObjectID.createFromTime(measurement.stop/1000),
      filter = {_id: {}},
      fields = {_id: 1};

  // Copy any expression filters into the query object.
  expression.filter(filter);
  filter._id.$gte = start;
  filter._id.$lt = stop;
  ...
  db.events(type, function (error, collection) {
    ...
    function handleResponse(error, cursor){
      ...
      cursor.each(function(error, row) {
        if (error) return callback(error);
        if (row) callback(error, new Event(type, row._id.getTimestamp(), row.d, row.d ? row.d.id : undefined));
        else {
          callback();
        }
      });
