...
function Measurement(expression, start, stop, tier){
  ...
  if (expression.type) {
    var reducer = reduces[expression.reduce];
    this.isPyramidal = reducer.pyramidal;

    if (!this.isPyramidal) {
      this.wrap = reducer.wrap;
      this.unwrap = reducer.unwrap;
    }
  } else {
    this.isPyramidal = false;
  }
...
Measurement.prototype.measure = function measure(db, callback) {
  var _this = this;
  if (_this.isGrouped) {
    var distinctExpression = require('../group-distinct')(_this.expression);
    var distinctMeasure = new Measurement(distinctExpression, _this.start, _this.stop, _this.tier);

    var group_names = Array.isArray(_this.isGrouped.groups) ? _this.isGrouped.groups : [];
    distinctMeasure.measure(db, function(metric) {
      metric.values.forEach(function(group) {
        if (group !== undefined && group !== null && group_names.indexOf(group) === -1) {
          group_names.push(group);
        }
      });
    });
    distinctMeasure.on('complete', function() {
...
function findOrComputeUnary(db, callback) {
  ...
  function find(measurement, callback) {
    ...
    function computeFlat(start, stop) {
      var horizons = config.horizons, pastHorizon = {
        is: 'past_horizon',
        tier: tier,
