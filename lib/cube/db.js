...
function Db(){
  ...
  function open(callback){
    ...
    if (mongoConfig["separate-events-database"])  q.defer(mongodb.Db.connect, url + '-events', options);

    // Open separate metrics database if configured
    if (mongoConfig["separate-metrics-database"]) q.defer(mongodb.Db.connect, url + '-metrics', options);
