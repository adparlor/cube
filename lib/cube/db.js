...
function mongoUrl(options) {
  ...
  var user = options["mongo-username"],
      ...
      auth = user ? user + ":" + pass + "@" : "",
      params = options["mongo-params"];
  return "mongodb://" + auth + host + ":" + port + "/" + name + (params ? "?"+params : "");
}
...
function Db(){
  ...
  function open(callback){
    ...
    var mongoConfig = config.mongodb,
      ...
      options = mongoOptions(mongoConfig);

    collection_prefix = mongoConfig["collection_prefix"] || '';
    metric_options = mongoConfig["mongo-metrics"];
    event_options  = mongoConfig["mongo-events"];

    metalog.info('mongo_connect', db.report());

    var q = queue();

    q.defer(mongodb.Db.connect, url, options);

    // Open separate events database if configured
    if (mongoConfig["separate-events-database"]) {
      var eventsUrl = url.replace(/(\?|$)/, '-events$1')
      q.defer(mongodb.Db.connect, eventsUrl, options);
    };

    // Open separate metrics database if configured
    if (mongoConfig["separate-metrics-database"]) {
      var metricsUrl = url.replace(/(\?|$)/, '-events$1')
      q.defer(mongodb.Db.connect, metricsUrl, options);
    }
    ...
    q.awaitAll(function(err, results) {
      ...
      var connId = 0;
      db_client = results[connId++];

      if (mongoConfig["separate-events-database"])  events_client = results[connId++];
      else events_client = db_client;

      if (mongoConfig["separate-metrics-database"]) metrics_client = results[connId++];
  ...
  function metrics_collection_factory(client){
    return function metrics(name, on_collection, tr){
      ...
      function on_m_create(clxn, oc_cb){
        clxn.ensureIndex({"i": 1, "_id.l": 1, "_id.t": 1, "_id.e": 1}, function(error){ oc_cb(error, clxn); });
      }
    };
  }
    ...
    return function events(name, on_collection, tr){
      ...
      function on_e_create(clxn, oc_cb){
        db.metrics(name, function also_make_metrics_clxn(error){
          ...
          // No index to make, the _id already contains the timestamp
          oc_cb(null, clxn);
        });
      }
    };
  ...
  function types(client) {
    return function(request, callback) {
      client.collectionNames(function(error, names) {
        handle(error);
        callback(names
                 .map(function(d)    { return d.name.replace('.','_');      })
