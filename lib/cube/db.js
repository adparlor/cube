...
function Db(){
  ...
  function open(options, callback){
    ...
    mongodb.Db.connect(mongoUrl(options), mongoOptions(options), function(error, dbHandle) {
      if (error) return callback(error);

      db_client = dbHandle;

      // Open separate events database if configured
      if (options["separate-events-database"])  events_client  = db_client.db(db_client.databaseName + '-events');
      else events_client = db_client;

      // Open separate metrics database if configured
      if (options["separate-metrics-database"]) metrics_client = db_client.db(db_client.databaseName + '-metrics');
