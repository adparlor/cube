...
exports.putter = function(db) {
  ...
  function putter(request, callback) {
    ...
    if (!type_re.test(type)) return callback({error: "invalid type '"+type+"'"}), -1;
    ...
    colls.events(type, function (events) {
      colls.metrics(type, function (metrics) {
        // Save any pending events to the collection.
...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    function query(callback) {
      collection.events(expression.type, function(events) {
        events.find(filter, fields, options, function(error, cursor) {
          handle(error);
