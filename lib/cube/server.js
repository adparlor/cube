...
var util           = require("util"),
    ...
    wrapper        = require("./callback-wrapper");
...
module.exports = function(options, db) {
    ...
    for (var i = -1, n = endpoints.ws.length, e; ++i < n;) {
      if ((e = endpoints.ws[i]).match(request.url)) {
        ...
        connection.on("message", function(message) {
          ...
          e.dispatch(payload, wrapper(connection_callback, payload.id));
        });
  ...
  server.start = function(server_start_cb) {
    ...
    // Start the server!
    function ready(db) {
      metalog.putter = event.putter(db, options);
      hasher.db = db;

      uses.forEach(function(adder){ adder(db, endpoints, options); });
