...
//
exports.register = function collector(db, endpoints, options) {
  var putter = require("./event").putter(db, options),
      poster = post(putter);
  ...
  endpoints.http.push(
    ...
    endpoint("POST", "/collectd", require("./collectd").putter(putter, options))
  );
