  ...
  function streamer(streamKey, measurement, callback) {
    ...
    if (stream) {
      ...
      measurement.on('complete', function() {
        ...
        measurement.stop = measurement.tier.step(measurement.stop);

        var nextRun = +measurement.stop + streamDelayDefault - Date.now();

        // setTimeout cannot take numbers bigger than this, and anyway it seems like a very long time to wait.
        var MAX_INT32 = 2147483647; // 2^31 - 1
        if (nextRun > MAX_INT32) return;
